...
@emp_sender
def device_info():
    return {'func': 'device_info', 'data': dict(
...
def memory_status():
    gc.collect()
    return {'func': 'memory_status', 'data': dict(alloced=gc.mem_alloc() / 1024, free=gc.mem_free() / 1024)}
...
def memory_analysing(filename):
    ...
    mf = gc.mem_free()
    return {'func': 'memory_analysing', 'data': dict(fsize=fsize, mf=mf, filename=filename)}
